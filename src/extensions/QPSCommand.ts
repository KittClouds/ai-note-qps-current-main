...
import { qpsService } from '@/lib/embeddings/quantum/qpsService';

export interface QPSSearchHit {
  id: string;
  title: string;
  preview: string;
  score?: number;
...
export const QPSCommand = Extension.create({
  name: 'qpsCommand',

  addOptions() {
    return {
      trigger: '@@@', // Unique trigger to avoid conflicts
      paletteKeys: ['Mod-Shift-k'], // Unique keyboard shortcut
      fetchHits: async (query: string): Promise<QPSSearchHit[]> => {
        if (!query.trim()) return [];
        
        try {
          const results = await qpsService.search(query, 5);
...
function renderList(items: QPSSearchHit[], selectedIndex: number, command: any) {
  ...
  if (items.length === 0) {
    ...
    emptyRow.textContent = 'No QPS results found';
    box.appendChild(emptyRow);
    return box;
  }

  items.forEach((item: QPSSearchHit, i: number) => {
    ...
    if (item.score) {
      ...
      scoreDiv.textContent = `QPS: ${item.score.toFixed(2)}`;
