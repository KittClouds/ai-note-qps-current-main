...
import { tokenizer } from './embeddingUtils';
import { cosineSimilarity } from './similarityUtils';
import { createEmbedding } from './embeddingUtils';
...
export async function optimizeAndRebalanceChunks(
  ...
  maxTokenSize: number, 
  combineChunksSimilarityThreshold: number = 0.5
): Promise<string[]> {
  ...
  for (let index = 0; index < combinedChunks.length; index++) {
    ...
    const chunkTokenCount = tokenizerInstance(chunk).input_ids.size;
...
export function applyPrefixToChunk(chunkPrefix: string | null, chunk: string): string {
