...
import { initializeEmbeddingUtils, tokenizer, createEmbedding } from './embeddingUtils';
import { computeAdvancedSimilarities, adjustThreshold } from './similarityUtils';
import { createChunks, optimizeAndRebalanceChunks, applyPrefixToChunk } from './chunkingUtils';
...
const VERSION = "1.0.0"; // Browser version
...
export async function cramit(
  documents: DocumentInput[],
  options: Omit<ChunkitOptions, 'similarityThreshold' | 'dynamicThresholdLowerBound' | 'dynamicThresholdUpperBound' | 'numSimilaritySentencesLookahead' | 'combineChunks' | 'combineChunksSimilarityThreshold'> = {}
): Promise<ChunkResult[]> {
  const {
    logging = DEFAULT_CONFIG.LOGGING,
    maxTokenSize = DEFAULT_CONFIG.MAX_TOKEN_SIZE,
...
export async function sentenceit(
  documents: DocumentInput[],
  options: Pick<ChunkitOptions, 'logging' | 'onnxEmbeddingModel' | 'dtype' | 'localModelPath' | 'modelCacheDir' | 'returnEmbedding' | 'returnTokenLength' | 'chunkPrefix' | 'excludeChunkPrefixInResults'> = {}
